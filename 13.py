data = open("data/13.data").read().split("\n\n")
...
def solve():
    ...
    for i in range(len(paper[0]) // 5):
        temp = []
        for j in range(4):
            temp.append("".join([paper[k][i*5+j] for k in range(6)][::-1]).replace("#", "1").replace(".", "0"))
    ...
    after_all = "".join(decoder[coded] for coded in coded_list)
