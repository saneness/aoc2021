data = [item.split() for item in open("data/22.data").read().split("\n")]
...
def solve():
    ...
    def reboot(init=False):
        ...
        return sum([numpy.prod([cube[i][1] + 1 - cube[i][0] for i in range(3)]) for cube in cubes_on])

    print(reboot(init=True))
    print(reboot())
