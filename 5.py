data = [[int(subitem) for subitem in item.split(' -> ')[0].split(',')] + [int(subitem) for subitem in item.split(' -> ')[1].split(',')] for item in open("data/5.data").read().split('\n')]
...
def solve():
    ...
    print(overlaps(diagonal=False))
    print(overlaps(diagonal=True))
