data = [item for item in open("data/4.data").read().split('\n\n')]
...
def solve():
    ...
    first = first_number * sum([item for item in first_bingo if item > 0])
    last = winner_number * sum([item for item in winner_bingo if item > 0])
